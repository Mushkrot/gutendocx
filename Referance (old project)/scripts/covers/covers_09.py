...
# Определяем рабочий путь (для создания/чтения config.json)
...
# Если запускается без флага "--config", запускаем конфигурационный режим в новом окне
if "--config" not in sys.argv:
    subprocess.run([exe_path, "--config"], creationflags=subprocess.CREATE_NEW_CONSOLE)

# Если программа запущена с флагом "--config", то запускаем конфигурационное окно и выходим.
...
# Если config.json еще не существует, создаем его с настройками по умолчанию.
...
BACKGROUND_IMAGE_PATH = None  # Будет задан позже
LOGO_PATH = None              # Будет задан позже
...
# Шрифты
SELECTED_FONT_NAME = config.get("SELECTED_FONT_NAME", "EuphoriaScript-Regular.ttf")
FONT_FILE = FONTS_FOLDER / SELECTED_FONT_NAME
SECOND_FONT_NAME = config.get("SECOND_FONT_NAME", "GreatVibes-Regular.ttf")
SECOND_FONT_FILE = FONTS_FOLDER / SECOND_FONT_NAME
ALTERNATIVE_FONT = config.get("ALTERNATIVE_FONT", "arial.ttf")

# Динамические параметры
ZONE_COUNT = config.get("ZONE_COUNT", 4)
ZONE_HEIGHT = IMAGE_HEIGHT / ZONE_COUNT
...
ALLOW_ZONE4_OVERLAP_RATIO = config.get("ALLOW_ZONE4_OVERLAP_RATIO", 0.5)

# Цвета (парсим строковое представление кортежей)
...
if isinstance(TEXT_COLOR, str):
    TEXT_COLOR = ast.literal_eval(TEXT_COLOR)
...
def create_image(data, output_path):
    try:
        ...
        title_area_height = zone_height - TITLE_TOP_MARGIN
        ...
        anchor_font_size, anchor_lines, anchor_total_height = find_optimal_font_size_for_text(
            anchor_text,
            IMAGE_WIDTH * ANCHOR_AREA_WIDTH_FACTOR, zone_height,
