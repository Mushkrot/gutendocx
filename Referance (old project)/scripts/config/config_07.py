# covers.py v2.9 - Title vertical alignment
# -*- coding: utf-8 -*-
...
# Обновлённый и отсортированный по алфавиту список шрифтов (без учёта регистра)
FONT_OPTIONS = sorted([
...
# Вычисляем максимальную длину названия шрифта для ширины OptionMenu
max_font_length = max(len(font) for font in FONT_OPTIONS) + 2

# Значения конфигурации по умолчанию (цвета представлены строками с кортежами)
DEFAULT_CONFIG = {
    # Title Settings
    ...
    # Новый параметр для вертикального выравнивания заголовка
...
# Определяем порядок ключей, сгруппированных по темам
KEYS_ORDER = [
    # Title Settings
    ...
    "TITLE_ZONE_PERCENT",
    "TITLE_TEXT_COLOR",
    "TITLE_VERTICAL_ALIGN",  # Новый параметр для вертикального выравнивания заголовка
...
class ConfigGUI(tk.Tk):
    def __init__(self):
        ...
        # Верхняя метка с версией программы
        version_label = tk.Label(self, text="Covers Creator ver 1.2", font=("Arial", 14, "bold"))
        version_label.pack(pady=10)
        
        # Фрейм для размещения настроек
        ...
        for idx, key in enumerate(KEYS_ORDER):
            ...
            # Для некоторых ключей используем выпадающее меню
            if key in ["TITLE_FONT_NAME", "SECONDARY_FONT_NAME", "ALTERNATIVE_FONT"]:
                ...
                opt = tk.OptionMenu(options_frame, var, *FONT_OPTIONS)
                opt.config(width=max_font_length, anchor="w")
                opt.grid(row=row, column=col+1, padx=5, pady=2, sticky="w")
            # Специально для TITLE_VERTICAL_ALIGN – выпадающее меню с фиксированными вариантами
