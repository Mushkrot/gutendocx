...
- Body-start detection retains the legacy break rules but can recover substantive post-cover text before an incorrectly chosen second-break boundary when recognized cover styles provide a safe anchor. Blank intervals, decorative markers, and documents without recognized cover styles keep legacy behavior.
...
- **2026-07-11:** Fixed a second class of stubborn 18pt files in the client's July 9 four-file batch. The payload correctly contained `Body.size_pt=12`, but `_compute_body_start_index()` treated a body-internal second break as the end of a blank page and excluded a first story, preface, biography/poems, or introductory section. A cover-anchored fallback now moves the boundary earlier only when a recognized cover style exists and at least 40 visible characters occur before the legacy boundary; genuine blank intervals, short decorative markers, and files without recognized cover styles preserve legacy behavior. The existing trailing-break rule also now tolerates empty service paragraphs after the break while still rejecting visible or structural content, keeping the `pg1868` line-spacing guard active. Real-file body starts changed from `438/46/145/93` to `8/7/12/9`; first substantive PDF body text changed from 18pt to 12pt, page counts only decreased (`272->234`, `318->305`, `218->197`, `128->100`), and TOC/heading structure stayed stable. Real `pg60112/60115/61492/61506` regressions passed, full pytest passed with 53 tests, Python compilation and diff checks passed. Commit `91b398d` was pushed; after confirming zero active jobs, production restarted with all required services active, localhost-only port 8000, healthy local endpoint, and Cloudflare Access redirect preserved. Live localhost `/whole/apply` returned the recovered synthetic boundary (`body_start_index=1`) with Body 12 applied. Runtime `config.yaml` and client artifacts remain uncommitted.
